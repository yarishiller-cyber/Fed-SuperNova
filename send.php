<?php
/** lead.php — contact handler. Emails info@arpa-e-simulation-funding.com server-side, logs every lead. */
header('Content-Type: application/json; charset=utf-8');
header('X-Content-Type-Options: nosniff');
header('Cache-Control: no-store');
$INBOX = 'info@arpa-e-simulation-funding.com'; $FROM = 'info@arpa-e-simulation-funding.com';
...
$lines=['New enquiry from arpa-e-simulation-funding.com','','Name:            '.$name,'Email:           '.$email,'Phone:           '.($phone!==''?$phone:'(not given)'),'Funding interest:'.($interest!==''?' '.$interest:' (not given)'),'','Message:',($msg!==''?$msg:'(none)'),'','Received: '.gmdate('Y-m-d H:i:s').' UTC · IP '.($_SERVER['REMOTE_ADDR']??'?')];
$body=implode("\r\n",$lines);
$dir=__DIR__.'/leads';
...
$h=['From: Watt Ridge Simulation Solutions Website <'.$FROM.'>','Reply-To: '.ch($name).' <'.ch($email).'>','MIME-Version: 1.0','Content-Type: text/plain; charset=UTF-8','Date: '.date('r'),'X-Mailer: Fleet-Form'];
$hdr=implode("\r\n",$h);
